...
export function PillsRow({ setActive, active, pills }: IpillsRowProps) {
  return (
    <div
      id="pills-row"
      className="flex flex-row items-start  gap-3 overflow-x-scroll border-b border-[#5555552a] bg-gray-1 px-6 py-8 sm:max-h-[96px] sm:overflow-x-clip sm:px-16 "
    >
      {pills.map((pillName) => (
        <Pill
          key={pillName}
    ...
    <div
      ...
      className={`flex cursor-pointer items-center justify-center rounded-[12px] border px-5 py-1 shadow-sm ${
        active === title
          ...
          : "border-[#d1d1d1] bg-white text-dark-2"
      }`}
