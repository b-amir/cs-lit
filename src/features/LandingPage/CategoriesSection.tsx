  ...
  return (
    ...
    >
      <Image
        ...
        alt="lighthouse"
      />
      <div className=" my-4 flex w-full flex-col text-[#E6E6E6] sm:right-0 sm:my-10 sm:min-h-screen md:max-w-[calc(5*100vw/5)] lg:right-0 lg:mx-24 lg:ml-auto lg:max-w-[calc(3*100vw/5)]">
        <h2
          className={`${archivo.className} h-3/12 mx-auto my-14 flex min-h-[calc(15dvh)] w-full flex-row justify-center text-balance px-6 text-center text-3xl font-bold [text-shadow:_0px_1px_5px_rgb(66_80_99_/_45%)] [text-shadow:_1px_1px_0px_rgb(66_80_99_/_30%)] sm:text-4xl`}
        >
        ...
        {categoriesFetchingStatus === "loading" ? (
          ...
          <ul className="h-8/12 mx-auto mb-28 grid w-full grid-cols-2 justify-center gap-6 px-4 text-lg font-normal sm:mb-20 lg:grid-cols-3">
              ...
              page?.items?.map((category) => (
                <li key={category.id} className="">
                  ...
                  >
                    ...
                    <span className="ml-4 flex-1 truncate text-ellipsis whitespace-nowrap text-sm font-semibold transition-all duration-200 sm:text-lg ">
                      {category.name}
                    </span>
                  </Link>
                </li>
        ...
        <div className=" h-1/12 absolute bottom-0 left-[5%] right-[5%] my-12 flex flex-col items-center sm:items-end">
          <p className="font-regular z-10 hidden w-[max-content] flex-row items-center justify-center gap-1.5 rounded-lg border border-[#d5d9df33] bg-[#d6e2f634] px-4 py-2 text-sm text-[#292626a9] shadow-sm backdrop-blur-sm sm:flex ">
