...
import { getScreenName, getScreenTitle } from "@/utils/getScreenName";

export function LatestCommentsSection() {
  const { data: latestComments, status: commentsFetchingStatus } =
  ...
  return (
    <div className="col-span-3 flex w-full flex-col sm:col-span-2 sm:border-l sm:pl-4">
      <div className="ml-1 flex w-full items-center justify-start gap-1.5 pb-6 pt-2 text-sm font-bold text-gray-800 ">
        <span className="flex select-none items-center gap-1.5 rounded-3xl border border-neutral-300 bg-neutral-200 px-4 py-2 text-xs font-semibold">
          <CommentIcon className="mb-0.5" /> Recent Comments
        </span>
      </div>
      ...
      {commentsFetchingStatus === "loading" ? (
        ...
        <div className="flex flex-col items-start overflow-clip">
          {latestComments?.pages?.map((page) =>
            page?.items?.map((item) => (
              <Link
                href={`${routeHandler(item, "Comments")}`}
              ...
              >
                <h1 className="max-w-[65%] truncate font-semibold text-gray-700">
                  {getScreenTitle(item)}
                </h1>
                <p className="max-w-[28%] truncate text-xs text-gray-500">
                  by {getScreenName(item?.user)}
                </p>
              </Link>
            ))
          )}
